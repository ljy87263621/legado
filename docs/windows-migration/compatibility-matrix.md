...
| WebDAV | HTTP(S) PROPFIND/PUT/GET for safe `backup*.zip` files, configuration persistence, restore and selected remote-book download/import | `WebDavBackupModelTest`, `WebDavSettingsModelTest`, SQLite restart tests | desktop-partial | No bidirectional progress/image synchronization or conflict resolution |
