...
| Browser verification | Validated HTTP(S) URL opens a JavaFX embedded browser; explicit completion stores cookies and re-fetches through the source-aware HTTP client; request debugging, search, book detail/TOC refresh and text reading classify login-required responses and expose an explicit re-login action | `EmbeddedBrowserVerificationModelTest`, `SourceModelTest`, `SourceSessionAssessmentTest`, `CoreSourceSessionTest`, desktop model tests | desktop-partial | Detailed follow-up design is in `source-browser-session-design.md`; automatic re-login, online-image/discovery/RSS integration, WebView2, CAPTCHA callback, `webJs` runtime and Android result-return semantics remain absent |
