...
| Browser verification | Validated HTTP(S) URL opens a JavaFX embedded browser; explicit completion stores cookies and re-fetches through the source-aware HTTP client; request debugging classifies session states and opens a detected login URL as an explicit re-login action | `EmbeddedBrowserVerificationModelTest`, `SourceModelTest`, `SourceSessionAssessmentTest` | desktop-partial | Detailed follow-up design is in `source-browser-session-design.md`; automatic re-login and integration into search, detail, TOC and content requests, plus WebView2, CAPTCHA callback, `webJs` runtime and Android result-return semantics remain absent |
