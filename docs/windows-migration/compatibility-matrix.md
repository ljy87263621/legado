...
| Search and discovery (`SearchActivity`, explore UI) | Source selection, `exploreUrl`, static categories, pagination, deduplication, source-filter rules, source variables and source-aware Cookies; discovery requests classify login-required responses and expose an explicit re-login action | `ExploreModelTest`, `BookSourceSearchTest`, `CoreSourceFilterServiceTest`, `CoreSourceRuntimeDataTest` | desktop-partial | Dynamic explore scripts, source ranking, WebView session sharing and full Android cache behavior remain absent |
...
| Browser verification | Validated HTTP(S) URL opens a JavaFX embedded browser; explicit completion stores cookies and re-fetches through the source-aware HTTP client; request debugging, search, book detail/TOC refresh, text reading, online image chapter/page loading and discovery classify login-required responses and expose an explicit re-login action | `EmbeddedBrowserVerificationModelTest`, `SourceModelTest`, `SourceSessionAssessmentTest`, `CoreSourceSessionTest`, desktop model tests | desktop-partial | Detailed follow-up design is in `source-browser-session-design.md`; automatic re-login, RSS integration, WebView2, CAPTCHA callback, `webJs` runtime and Android result-return semantics remain absent |
